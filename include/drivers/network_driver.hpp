...
public:
  ...
  virtual void send(std::vector<unsigned char> data, bool losable) = 0;
...
public:
  ...
  void disconnect();
  void send(std::vector<unsigned char> data, bool losable);
...
private:
  ...
  std::vector<std::vector<unsigned char>> lagged_msg;
};
